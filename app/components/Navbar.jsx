...
const Navbar = () => {
    ...
    useEffect(() => {
        const handleClickOutsideMobile = (event) => {
            if (
                (collectionMobileRef.current && !collectionMobileRef.current.contains(event.target)) &&
                (aboutMobileRef.current && !aboutMobileRef.current.contains(event.target))
            ) {
    ...
    return (
        <header className={`flex z-20 fixed top-0 items-center justify-between py-5 font-medium w-full left-0 px-6 md:px-16 lg:px-32 transition-all duration-300 ${scrolled ? "bg-white shadow-md py-8" : "bg-transparent"}`}>
            <Link href="/">
            ...
            <ul className="hidden sm:flex gap-5 text-sm text-gray-700">
                {["/", "/collection", "/shop", "/about", "/contact"].map((path, index) => {
                    ...
                    if (path === "/about") {
                        return (
                            <div key={index} className="relative" ref={aboutRef}>
                                <div
                                    onClick={() => {
                                ...
                                >
                                    <div className="flex items-center">
                                        ...
                                        <Image
                                            src={assets.arrow_drop_down}
                                            alt="Dropdown Arrow"
                                            className={`w-5 transition-transform duration-300 ${isRotatedAbout ? "rotate-180" : "rotate-0"}`}
                                        />
                                    </div>
                                    <hr className={`w-0 border-none h-[1.5px] bg-black transition-all duration-500 ${isActive ? "w-2/4" : "w-0"}`} />
                                </div>
                                ...
                                {isAboutOpen && (
                                    <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-200 rounded shadow-lg z-50 transition-all duration-300">
                                        <Link href="/about/team">
                                            <p className="block px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer">Our Team</p>
                                        </Link>
                                        <Link href="/about/mission">
                                            <p className="block px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer">Our Mission</p>
                                        </Link>
                                    </div>
                                )}
                            </div>
                    ...
                    return (
                        <Link key={index} href={path}>
                            <div className="flex flex-col items-center gap-1 cursor-pointer text-gray-400 hover:text-black">
                                <p className={`transition-all duration-500 ${isActive ? "text-black" : "text-gray-400"}`}>{labels[index]}</p>
                                <hr className={`w-0 border-none h-[1.5px] bg-black transition-all duration-500 ${isActive ? "w-2/4" : "w-0"}`} />
                            </div>
                        </Link>
                    );
            ...
            <div className="flex items-center gap-6">
                <div className="flex items-center md:hidden gap-3">
                    {isSeller && (
                        <button onClick={() => router.push("/seller")} className="text-xs bg-emerald-600 text-white px-4 py-2 rounded-sm">
                            Admin Dashboard
                        </button>
                    )}
                </div>
            ...
            >
                <div className="flex flex-col text-gray-600">
                    ...
                    {["/", "/collection", "/shop", "/about", "/contact"].map((path, index) => {
                        ...
                        if (path === "/collection") {
                            return (
                                <div key={index} className="relative" ref={collectionMobileRef}>
                                    <div
                                        onClick={() => {
                        ...
                        if (path === "/about") {
                            return (
                                <div key={index} className="relative" ref={aboutMobileRef}>
                                    <div
                                        onClick={() => {
